...
export default function ({ app, store, route, error, redirect, hotReload }) {
  ...
  // console.log('test locale in url' + route.fullPath.indexOf(`/${DEFAULT_LOCALE}`))
  ...
  if (locale === DEFAULT_LOCALE && route.fullPath.indexOf(`/${DEFAULT_LOCALE}`) === 0) {
    ...
    //
    //   params[entry[0]] = app.i18n.t(entry[1], locale)
    //   // console.log('key : ' + entry[0])
    //   // console.log('value translate: ' + app.i18n.t(entry[1], locale))
    // })
    // // console.log(params)
    // app.router.push({ name: route.name, params: params })
  }
}
